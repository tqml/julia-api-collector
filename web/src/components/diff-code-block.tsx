...
export function DiffCodeBlock({ diff, show, showNormal = true }: DiffCodeBlockProps) {
    return (
        <pre
            ...
            <code>
                {diff.map((line, i) => {
                    if (line.type === "+" && show === "additions") {
                        return (
                            <div key={i} className="bg-green-500/10 text-green-600 dark:bg-green-900/30 dark:text-green-400">
                                + {line.content}
                            </div>
                        )
                    }
                        ...
                        return (
                            <div key={i} className="bg-red-500/10 text-red-600 dark:bg-red-900/30 dark:text-red-400">
                                - {line.content}
                            </div>
                        )
                    }
